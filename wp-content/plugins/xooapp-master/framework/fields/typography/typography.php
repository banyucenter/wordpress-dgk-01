 ...
 *
 */
class XOOAPPFramework_Option_typography extends XOOAPPFramework_Options {
  ...
  public function output() {
    ...
    $default_variants = apply_filters( 'XOOAPP_websafe_fonts_variants', array(
      'regular',
      'italic',
      '700',
      '700italic',
    ...
    $websafe_fonts = apply_filters( 'XOOAPP_websafe_fonts', array(
      'Arial',
      'Arial Black',
      'Comic Sans MS',
      'Impact',
    ...
    $google_json   = XOOAPP_get_google_fonts();
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<label class="xooapp-typography-family">';
      echo '<select name="'. $this->element_name( '[family]' ) .'" class="'. $is_chosen . $chosen_rtl .'xooapp-typo-family" data-atts="family"'. $this->element_attributes() .'>';

      do_action( 'XOOAPP_typography_family', $family_value, $this );

      echo '<optgroup label="'. esc_html__( 'Web Safe Fonts', 'xooapp' ) .'">';
      ...
      echo '<optgroup label="'. esc_html__( 'Google Fonts', 'xooapp' ) .'">';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        echo '<label class="xooapp-typography-variant">';
        echo '<select name="'. $this->element_name( '[variant]' ) .'" class="'. $is_chosen . $chosen_rtl .'xooapp-typo-variant" data-atts="variant">';
      ...
      echo '<input type="text" name="'. $this->element_name( '[font]' ) .'" class="xooapp-typo-font hidden" data-atts="font" value="'. $value['font'] .'" />';

    } else {

      echo esc_html__( 'Error! Can not load json file.', 'xooapp' );

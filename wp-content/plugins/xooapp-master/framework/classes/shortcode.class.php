 ...
 *
 */
class XOOAPPFramework_Shortcode_Manager extends XOOAPPFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'XOOAPP_shortcode_options', $options );
    $this->exclude_post_types = apply_filters( 'XOOAPP_shortcode_exclude', $this->exclude_post_types );
    ...
    if( ! empty( $this->options ) ) {
      ...
      $this->addAction( 'wp_ajax_xooapp-get-shortcode', 'shortcode_generator', 99 );
  ...
  public static function instance( $options = array() ){
    if ( is_null( self::$instance ) && XOOAPP_ACTIVE_SHORTCODE ) {
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary xooapp-shortcode" data-editor-id="'. $editor_id .'">'. esc_html__( 'Add Shortcode', 'xooapp' ) .'</a>';
    }
  ...
  ?>
    <div id="xooapp-shortcode-dialog" class="xooapp-dialog" title="<?php esc_html_e( 'Add Shortcode', 'xooapp' ); ?>">
      <div class="xooapp-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>xooapp-dialog-select" data-placeholder="<?php esc_html_e( 'Select a shortcode', 'xooapp' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="xooapp-dialog-load"></div>
      <div class="xooapp-insert-button hidden">
        <a href="#" class="button button-primary xooapp-dialog-insert"><?php esc_html_e( 'Insert Shortcode', 'xooapp' ); ?></a>
      </div>
    </div>
  <?php
  }
  ...
  public function shortcode_generator() {
    ...
    $request = XOOAPP_get_var( 'shortcode' );
    ...
    if( isset( $shortcode['fields'] ) ) {
      ...
      foreach ( $shortcode['fields'] as $key => $field ) {
        ...
        echo xooapp_add_element( $field, $field_default, 'shortcode' );
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      echo '<div class="xooapp-shortcode-clone" data-clone-id="'. $clone_id .'">';
      echo '<a href="#" class="xooapp-remove-clone"><i class="fa fa-trash"></i></a>';
      ...
      echo '<div class="xooapp-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. $shortcode['clone_title'] .'</a></div>';

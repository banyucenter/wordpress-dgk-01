 ...
 *
 */
abstract class XOOAPPFramework_Options extends XOOAPPFramework_Abstract {
  ...
  public function element_after() {
    ...
    $out  = ( isset( $this->field['info'] ) ) ? '<p class="xooapp-text-desc">'. $this->field['info'] .'</p>' : '';
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'XOOAPP_OPTIONS_DEBUG' ) && XOOAPP_OPTIONS_DEBUG ) ) {

      $value = $this->element_value();

      $out .= "<pre>";
      $out .= "<strong>". esc_html__( 'CONFIG', 'xooapp' ) .":</strong>";
      $out .= "\n";
      ob_start();
      var_export( $this->field );
      $out .= htmlspecialchars( ob_get_clean() );
      $out .= "\n\n";
      $out .= "<strong>". esc_html__( 'USAGE', 'xooapp' ) .":</strong>";
      $out .= "\n";
      $out .= ( isset( $this->field['id'] ) ) ? "XOOAPP_get_option( '". $this->field['id'] ."' );" : '';

      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". esc_html__( 'VALUE', 'xooapp' ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
    ...
    if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'XOOAPP_OPTIONS_DEBUG_LIGHT' ) && XOOAPP_OPTIONS_DEBUG_LIGHT ) ) {
      ...
      $out .= ( isset( $this->field['id'] ) ) ? "XOOAPP_get_option( '". $this->field['id'] ."' );" : '';
      $out .= "\n";
      $out .= "<strong>". esc_html__( 'ID', 'xooapp' ) .":</strong>";
      $out .= "\n";
  ...
  public function element_get_error() {
    ...
    if( ! empty( $XOOAPP_errors ) ) {
      foreach ( $XOOAPP_errors as $key => $value ) {
        if( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
          $out .= '<p class="xooapp-text-warning">'. $value['message'] .'</p>';
        }
      }
    }
  ...
  public function element_help() {
    return ( isset( $this->field['help'] ) ) ? '<span class="xooapp-help" data-title="'. $this->field['help'] .'"><span class="fa fa-question-circle"></span></span>' : '';
  }
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      foreach ( $this->multilang['languages'] as $key => $val ) {
        ...
        if( $key != $this->multilang['current'] ) {
          ...
          $class = 'XOOAPPFramework_Option_' . $this->field['type'];
      ...
      $out .= '</fieldset>';
      $out .= '<p class="xooapp-text-desc">'. sprintf( esc_html__( 'You are editing language: ( %s )', 'xooapp' ), '<strong>'. $this->multilang['current'] .'</strong>' ) .'</p>';
  ...
  public function element_multilang() {
    return ( isset( $this->field['multilang'] ) ) ? XOOAPP_language_defaults() : false;
  }

}

// load all of fields
XOOAPP_load_option_fields();

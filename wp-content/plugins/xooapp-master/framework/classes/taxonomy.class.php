 ...
 *
 */
class XOOAPPFramework_Taxonomy extends XOOAPPFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'XOOAPP_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && XOOAPP_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = XOOAPP_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $XOOAPP_errors = get_transient( 'xooapp-taxonomy-transient' );

    wp_nonce_field( 'xooapp-taxonomy', 'xooapp-taxonomy-nonce' );

    echo '<div class="xooapp-framework xooapp-taxonomy xooapp-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo xooapp_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( XOOAPP_get_var( 'xooapp-taxonomy-nonce' ), 'xooapp-taxonomy' ) ) {

      $errors = array();
      $taxonomy = XOOAPP_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'XOOAPP_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'XOOAPP_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'XOOAPP_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'XOOAPP_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'XOOAPP_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'xooapp-taxonomy-transient', $errors, 10 );

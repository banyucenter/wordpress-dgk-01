...
function xooapp_init_minify_html() {
    $minify_html_active = get_option( 'minify_html_active' );
    if ( $minify_html_active != 'no' )
        ob_start('xooapp_minify_html_output');
}
if ( !is_admin() )
    if ( !( defined( 'WP_CLI' ) && WP_CLI ) )
        add_action( 'init', 'xooapp_init_minify_html', 1 );

function xooapp_minify_html_output($buffer) {
...
function xooapp_minify_html_menu() {
    add_options_page( 'Minify HTML Options', 'Minify HTML', 'manage_options', 'minify_html_options', 'xooapp_minify_html_menu_options' );
}
add_action( 'admin_menu', 'xooapp_minify_html_menu' );

function xooapp_minify_html_menu_options() {
    ...
    if ( isset($_POST[ 'minify_html_submit_hidden' ]) && $_POST[ 'minify_html_submit_hidden' ] == 'Y' ) {
        ...
        echo '<div class="updated"><p><strong>' . __( 'Settings saved.', 'xooapp' ) . '</strong></p></div>';
    }
?>
<style>
        ...
        <table class="form-table">
            <tbody>
                <tr class="minify_html_active">
                    <th><label><?php echo __( 'Minify HTML', 'xooapp' ); ?></label></th>
                    <td>
                        <input type="radio" name="minify_html_active" value="yes"<?php echo ($minify_html_active=='yes' ? ' checked' : ''); ?>><span class="value"><strong><?php _e( 'Enable', 'xooapp' ); ?></strong></span>
                        <input type="radio" name="minify_html_active" value="no"<?php echo ($minify_html_active!='yes' ? ' checked' : ''); ?>><span class="value"><?php _e( 'Disable', 'xooapp' ); ?></span>
                        <p class="description"><?php echo __( 'Enable or disable Minify HTML', 'xooapp' ); ?></p>
                    </td>
                </tr>
                <tr class="minify_javascript minify_html_options">
                    <th><label><?php echo __( 'Minify inline JavaScript', 'xooapp' ); ?></label></th>
                    <td>
                        <input type="radio" name="minify_javascript" value="yes"<?php echo ($minify_javascript=='yes' ? ' checked' : ''); ?>><span class="value"><strong><?php _e( 'Yes', 'xooapp' ); ?></strong></span>
                        <input type="radio" name="minify_javascript" value="no"<?php echo ($minify_javascript!='yes' ? ' checked' : ''); ?>><span class="value"><?php _e( 'No', 'xooapp' ); ?></span>
                        <p class="description"><?php echo __( 'This option is typically safe to set to "Yes"', 'xooapp' ); ?></p>
                    </td>
                </tr>
                <tr class="minify_html_comments minify_html_options">
                    <th><label><?php echo __( 'Remove HTML, JavaScript and CSS comments', 'xooapp' ); ?></label></th>
                    <td>
                        <input type="radio" name="minify_html_comments" value="yes"<?php echo ($minify_html_comments=='yes' ? ' checked' : ''); ?>><span class="value"><strong><?php _e( 'Yes', 'xooapp' ); ?></strong></span>
                        <input type="radio" name="minify_html_comments" value="no"<?php echo ($minify_html_comments!='yes' ? ' checked' : ''); ?>><span class="value"><?php _e( 'No', 'xooapp' ); ?></span>
                        <p class="description"><?php echo __( 'This option is typically safe to set to "Yes"', 'xooapp' ); ?></p>
                    </td>
                </tr>
                <tr class="minify_html_xhtml minify_html_options">
                    <th><label><?php echo __( 'Remove XHTML closing tags from HTML5 void elements', 'xooapp' ); ?></label></th>
                    <td>
                        <input type="radio" name="minify_html_xhtml" value="yes"<?php echo ($minify_html_xhtml=='yes' ? ' checked' : ''); ?>><span class="value"><?php _e( 'Yes', 'xooapp' ); ?></span>
                        <input type="radio" name="minify_html_xhtml" value="no"<?php echo ($minify_html_xhtml!='yes' ? ' checked' : ''); ?>><span class="value"><strong><?php _e( 'No', 'xooapp' ); ?></strong></span>
                        <p class="description"><?php echo __( 'This option is typically safe to set to "Yes"', 'xooapp' ); ?></p>
                    </td>
                </tr>
                <tr class="minify_html_relative minify_html_options">
                    <th><label><?php echo __( 'Remove relative domain from internal URLs', 'xooapp' ); ?></label></th>
                    <td>
                        <input type="radio" name="minify_html_relative" value="yes"<?php echo ($minify_html_relative=='yes' ? ' checked' : ''); ?>><span class="value"><?php _e( 'Yes', 'xooapp' ); ?></span>
                        <input type="radio" name="minify_html_relative" value="no"<?php echo ($minify_html_relative!='yes' ? ' checked' : ''); ?>><span class="value"><strong><?php _e( 'No', 'xooapp' ); ?></strong></span>
                        <p class="description"><?php echo __( 'This option is typically safe to set to "Yes"', 'xooapp' ); ?></p>
                    </td>
                </tr>
                <tr class="minify_html_scheme minify_html_options">
                    <th><label><?php echo __( 'Remove schemes (HTTP: and HTTPS:) from all URLs', 'xooapp' ); ?></label></th>
                    <td>
                        <input type="radio" name="minify_html_scheme" value="yes"<?php echo ($minify_html_scheme=='yes' ? ' checked' : ''); ?>><span class="value"><?php _e( 'Yes', 'xooapp' ); ?></span>
                        <input type="radio" name="minify_html_scheme" value="no"<?php echo ($minify_html_scheme!='yes' ? ' checked' : ''); ?>><span class="value"><strong><?php _e( 'No', 'xooapp' ); ?></strong></span>
                        <p class="description"><?php echo __( 'This option is typically best to leave as "No"', 'xooapp' ); ?></p>
                    </td>
                </tr>
                <tr class="minify_html_utf8 minify_html_options">
                    <th><label><?php echo __( 'Support multi-byte UTF-8 encoding (if you see odd characters)', 'xooapp' ); ?></label></th>
                    <td>
                        <input type="radio" name="minify_html_utf8" value="yes"<?php echo ($minify_html_utf8=='yes' ? ' checked' : ''); ?>><span class="value"><?php _e( 'Yes', 'xooapp' ); ?></span>
                        <input type="radio" name="minify_html_utf8" value="no"<?php echo ($minify_html_utf8!='yes' ? ' checked' : ''); ?>><span class="value"><strong><?php _e( 'No', 'xooapp' ); ?></strong></span>
                        <p class="description"><?php echo __( 'This option is typically best to leave as "No"', 'xooapp' ); ?></p>
                    </td>
                </tr>
                </tr>
                    <td>&nbsp;</td>
                    <td>(<strong><?php echo __( 'Bold', 'xooapp' ); ?></strong> = <?php echo __( 'default value', 'xooapp' ); ?>)</td>
                </tr>
            </tbody>
        </table>
        <p class="submit">

...
function xooapp_like_plugin_scripts() {
	if ( SCRIPT_DEBUG == true || WP_DEBUG == true ) {	
		$min = '';
	} else {
		$min = '.min';
	}
	wp_enqueue_style('simpleLikescss', plugins_url( '/custom'.$min.'.css', __FILE__), false, false, 'all');
	wp_enqueue_script( 'simple-likes-js', plugins_url( '/custom'.$min.'.js', __FILE__ ), array( 'jquery' ), '0.5', true );
	wp_localize_script( 'simple-likes-js', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like'    => __( 'Like', 'xooapp' ),
		'unlike'  => __( 'Unlike', 'xooapp' )
	) ); 
}
add_action( 'wp_enqueue_scripts', 'xooapp_like_plugin_scripts' );
 ...
 * @since    0.5
 */
function xooapp_get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'xooapp' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'xooapp' );
		$icon = $icon_empty;
	}
...
} // xooapp_get_simple_likes_button()

/**
 * Processes shortcode to manually add the button to posts
 * @since    0.5
 */
add_shortcode( 'jmliker', 'xooapp_shortcode' );
function xooapp_shortcode() {
	return xooapp_get_simple_likes_button( get_the_ID(), 0 );
} // shortcode()
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'xooapp' ); ?></label></th>
			<td>
				<?php
				...
				if ( $like_query->have_posts() ) : ?>
					...
					<?php else : ?>
						<p><?php _e( 'You do not like anything yet.', 'xooapp' ); ?></p>
						<?php 
					endif; 
					wp_reset_postdata(); 
					?>
